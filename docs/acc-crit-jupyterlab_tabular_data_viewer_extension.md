...
- [ ] **Every shape of file that opened before still opens** - a blank column header, a blank row inside the data, an all-empty column, an empty first sheet, a blank row above the data and a header row with no data under it must not change the table's shape or fail the read
  ...
  - log: 2026-08-11 round-5 architect review: two more shapes found and fixed - a workbook with a broken external-link reference 500d on the sheet listing, and an all-NULL Parquet column 500d on statistics. A csv integer of 39+ digits still does not open (DEF-21). Criterion stays open
  - log: 2026-08-11 round-5 bug-hunter: a CRITICAL export data loss (case-clashing headers wrote a one-cell workbook at 200), an integer in the uint64 band demoted to text, and a 500 on statistics for any zero-row table - all three fixed. Numeric-text Excel columns (DEF-22) and a silent null past 2\*\*127 (DEF-23) recorded. Criterion stays open
  - log: 2026-08-11 DEF-22 and DEF-23 fixed on the user's instruction rather than recorded: a numeric-text column keeps its text (zip code leading zeros intact) but sorts numerically and reports aggregates via numeric_view; a spreadsheet integer past 2\*\*127 reads as text instead of a silent null. 148 pytest
  - log: 2026-08-11 round-6 architect: three defects introduced by the round-5 fixes, all fixed - the xlsx header suffix stole a name the file carried, parquet metadata reported null where statistics said string, and the xlsxwriter warning guard could not see the one path that truncates (16385 columns, now a 400 naming the grid limit). Numeric-text handling applies to every format, not only Excel
  - log: 2026-08-11 round-6 bug-hunter: three more defects from the round-5/6 fixes, all fixed - a numeric filter on a uint64 column was silently discarded and matched every row, an id past 2\*\*53 was rounded by JSON.parse and by the xlsx writer, and a text column holding Inf made the statistics response unparseable. Frontend gate corrected so a numeric-text column's aggregates reach the panel. DEF-24 recorded (categorical parquet, pre-existing)
